...
def graph():
	'''
	Shallow and wide baseline network.
	'''
	...
	for i in range(1,DEPTH+1):
		...
		current = tf.nn.max_pool(
					value  =current,
					ksize  =(1,MP[i-1],MP[i-1],1),
					strides=(1,MP[i-1],MP[i-1],1),
					padding="SAME"
				)

		chan_in       = i*CHANNELS
		height, width = height/MP[i-1], width/MP[i-1]

	current = batch_norm(current, (height, width, chan_in))
	current = conv(current, height, width, chan_in, 2, padding="VALID")
	current = leaky_relu(current)
...
if __name__ == '__main__':
	...
	labels, images, keep_prob, augment, prob, loss, train_step = graph()
